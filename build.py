...
def build_base():
    try:
        ...
        # Docker build
        subprocess.call('docker build -t we-baker:base -f Dockerfile.base .', shell=True)
    finally:
...
def build_local():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t we-baker:local -f Dockerfile.local .', shell=True)
    finally:
...
def build_dev():
    try:
        ...
        # docker build
        subprocess.call('docker build -t we-baker:dev -f Dockerfile.dev .', shell=True)
    finally:
        # 끝난 후 requirements.txt 파일 삭제
        os.remove('requirements.txt')
...
def build_production():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t we-baker:production -f Dockerfile.production .', shell=True)
    finally:
        # 끝난 후 requirements.txt 파일 삭제
        os.remove('requirements.txt')
